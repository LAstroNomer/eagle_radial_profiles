...
tab = pd.read_csv('Bar_params.csv')
names = tab['name'].to_numpy()
ells = tab['ell'].to_numpy()
no_bar = []
for name, ell in zip(names, ells):
    if ell == 0:
        no_bar.append(name)
print(no_bar)
...
for gal in gals:
    ...
    for num, j in enumerate(incs):
        ...
        if j == 0:
            ...
            # Fit ONE Break
            # --------------------------------
            if not(os.path.exists(f'fits/{file}/best_break.dat')):
                with open(f'fits/{file}/best_exp.dat', 'r') as ff:
            ...
            # --------------------------------------------
            '''
            if not(os.path.exists(f'fits/{file}/best_break_double.dat')):
