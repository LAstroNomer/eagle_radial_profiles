...
from azimProfile import main as azimp
...
if __name__ == '__main__':
    ...
    ell_file = f'{fname}_ellipse_flex.txt'
    ...
    for key in models:
        ...
        model_image = f'model_{key}.fits'
        azim_tab = f'{fname}_azim_0_model_{key}.txt'
        if not(os.path.exists(azim_tab)):
    ...
    for key in models:
        ...
        #fits.writeto(f'model_{key}.fits', models[key])
        model_image = f'model_{key}.fits'
        azim_tab = f'{fname}_azim_25_model_{key}.txt'
    ...
    sma_0, flux_0, flux_err_0 = np.loadtxt(f'{fname}_azim_0.txt', skiprows=1, unpack=True)
    #sma_0_model, flux_0_model, flux_err_0_model = np.loadtxt(f'{fname}_azim_0_model.txt', skiprows=1, unpack=True)
    sma_25, flux_25, flux_err_25 = np.loadtxt(f'{fname}_azim_25.txt', skiprows=1, unpack=True)
    ...
    for key in models:
        sma_0_model, flux_0_model, flux_err_0_model = np.loadtxt(f'{fname}_azim_0_model_{key}.txt', skiprows=1, unpack=True)
    ...
    for key in models:
        sma_25_model, flux_25_model, flux_err_25_model = np.loadtxt(f'{fname}_azim_25_model_{key}.txt', skiprows=1, unpack=True)
